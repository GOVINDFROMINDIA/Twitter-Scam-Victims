...
community_colors = [plt.cm.tab10(i) for i in range(len(communities))]
...
nx.draw(G, pos, with_labels=True, node_size=node_size, node_color=[partition[i] for i in G.nodes()], cmap=plt.cm.viridis, font_size=12, ax=ax, arrowsize=15, edge_color=edge_color)

# Adjust layout to prevent label overlap
plt.tight_layout()
legend_handles = [mpatches.Patch(color=color, label=f'Community {community_id}') for community_id, color in enumerate(community_colors)]

# Add legend
plt.legend(handles=legend_handles, title='Community', title_fontsize='large')
#plt.legend()
# Display the plot
plt.title('NetworkX Directed Graph with Node Color by Scam Percentage and Communities')
plt.show()
