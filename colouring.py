...
# Plot the NetworkX graph with node colors based on scam percentage
pos = nx.spring_layout(G)  # Layout the nodes

fig, ax = plt.subplots()
nx.draw(G, pos, with_labels=True, node_size=300, node_color=node_colors, font_size=10, ax=ax)
plt.title('NetworkX Graph Based on Connections (Self-loops removed) with Node Color by Scam Percentage')
plt.show()
